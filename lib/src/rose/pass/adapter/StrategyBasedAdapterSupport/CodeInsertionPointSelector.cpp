...
void _handleExceptionReturn(std::vector<std::pair<SgStatement *, CodeInsertionHelper::RelativePosition>> &stmtList,
                            SgStatement *node) {
...
void _handleContinueStatements(std::vector<std::pair<SgStatement *, CodeInsertionHelper::RelativePosition>> &stmtList,
                               SgStatement *node) {
...
void _handleBreakStatements(std::vector<std::pair<SgStatement *, CodeInsertionHelper::RelativePosition>> &stmtList,
                            SgStatement *node) {
...
void _transformReturnHelper(SgReturnStmt *node) {
  ...
  if (ex == NULL || isSgValueExp(ex) || isSgVarRefExp(ex)) {
    return;
  }
...
void _handleReturnStatements(std::vector<std::pair<SgStatement *, CodeInsertionHelper::RelativePosition>> &stmtList,
                             SgStatement *node) {
...
void _handleEndOfBasicBlock(std::vector<std::pair<SgStatement *, CodeInsertionHelper::RelativePosition>> &stmtList,
                            SgBasicBlock *node) {
  // Return statements are already considered
  if (isSgReturnStmt(SageInterface::getLastStatement(node)))
    return;
...
void _handleBasicBlockExitPoints(std::vector<std::pair<SgStatement *, CodeInsertionHelper::RelativePosition>> &stmtList,
                                 SgBasicBlock *node) {
  if (node == NULL)
    return;
...
_EnsureNthScopeAsBasicBlockVisitor::_EnsureNthScopeAsBasicBlockVisitor(size_t index) : index(index) {}
...
void _EnsureNthScopeAsBasicBlockVisitor::visitDefault(SgNode *node) { assert(("Unexpected statement input!", false)); }
...
CodeInsertionHelper getStatementEntryPoint(SgStatement *node) {
  ...
  if (functionBodies.count(node) > 0)
    return CodeInsertionHelper(std::vector<SgDeclarationStatement *>());
