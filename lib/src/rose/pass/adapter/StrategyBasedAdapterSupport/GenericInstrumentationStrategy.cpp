...
void applyNodesToCodeInsertionHelper(SgStatement *n, const std::vector<SgStatement *> &v,
                                     InsertionPointSelectorNthStmt sel, size_t scopeIndex) {
  if (v.size()) {
...
void applyNodesToCodeInsertionHelper(SgStatement *n, const std::vector<SgStatement *> &v,
                                     InsertionPointSelectorStmt sel) {
  _applyNodesToCodeInsertionHelper(n, v, sel);
}
...
void applyNodesToCodeInsertionHelper(SgNode *n, const std::vector<SgDeclarationStatement *> &v,
                                     InsertionPointSelectorNode sel) {
  _applyNodesToCodeInsertionHelper(n, v, sel);
}
void applyNodesToCodeInsertionHelper(SgStatement *n, const std::vector<SgDeclarationStatement *> &v,
                                     InsertionPointSelectorStmt sel) {
  _applyNodesToCodeInsertionHelper(n, v, sel);
}
void applyNodesToCodeInsertionHelper(SgNode *n, const std::vector<SgDeclarationStatement *> &v,
...
void GenericInstrumentationStrategy::applyInstrumentationFor(SgNode *node) {
  ...
  applyNodesToCodeInsertionHelper(node, constructToolDeclarationInLocalFile(global),
                                  &getEarliestGlobalDeclarationPoint);
  ...
  applyNodesToCodeInsertionHelper(node, constructInitializationInFunctionScopeFor(node),
                                  &getFunctionInitializationPoint);
  ...
  if (scope) {
    ...
    for (size_t i = 0; i < innerScopeCount; ++i) {
      applyNodesToCodeInsertionHelper(scope, constructExitInstrumentationInsideNthScopeFor(scope, i),
                                      &getNthInnerScopeExitPoints, i);
      applyNodesToCodeInsertionHelper(scope, constructEntryInstrumentationInsideNthScopeFor(scope, i),
                                      &getNthInnerScopeEntryPoint, i);
    }
  }
...
std::vector<SgDeclarationStatement *> GenericInstrumentationStrategy::constructToolDeclarationInLocalFile(
    SgGlobal *fileScope) {
...
std::vector<SgStatement *> GenericInstrumentationStrategy::constructEntryInstrumentationInsideNthScopeFor(
    SgScopeStatement *node, size_t scopeIndex) {
  return std::vector<SgStatement *>();
}
...
std::vector<SgStatement *> GenericInstrumentationStrategy::constructExitInstrumentationInsideNthScopeFor(
    SgScopeStatement *node, size_t scopeIndex) {
  return std::vector<SgStatement *>();
}
