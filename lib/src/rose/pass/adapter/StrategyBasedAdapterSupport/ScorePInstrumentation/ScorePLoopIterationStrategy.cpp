...
ScorePLoopIterationStrategy::ScorePLoopIterationStrategy(std::shared_ptr<ToolInitializer> initializer)
...
bool ScorePLoopIterationStrategy::isImplementedFor(SgNode *node) {
  ...
  if (!GenericInstrumentationStrategy::isImplementedFor(node))
    return false;
...
std::vector<SgStatement *> ScorePLoopIterationStrategy::constructInitializationInMainScopeFor(SgNode *node) {
  ...
  SgFunctionCallExp *call =
      SageBuilder::buildFunctionCallExp("SCOREP_USER_REGION_INIT", SageBuilder::buildVoidType(),
                                        SageBuilder::buildExprListExp(arg1, arg2, arg3), mainBody);
...
std::vector<SgStatement *> ScorePLoopIterationStrategy::constructEntryInstrumentationInsideNthScopeFor(
    SgScopeStatement *node, size_t index) {
  std::vector<SgStatement *> result;

  std::string handle = regionHandleName(node);

  SgExpression *call = SageBuilder::buildFunctionCallExp(
      "SCOREP_USER_REGION_ENTER", SageBuilder::buildVoidType(),
      SageBuilder::buildExprListExp(SageBuilder::buildOpaqueVarRefExp(handle, SageInterface::getGlobalScope(node))),
...
std::vector<SgStatement *> ScorePLoopIterationStrategy::constructExitInstrumentationInsideNthScopeFor(
    SgScopeStatement *node, size_t index) {
  std::vector<SgStatement *> result;
  ...
  SgExpression *call = SageBuilder::buildFunctionCallExp(
      "SCOREP_USER_REGION_END", SageBuilder::buildVoidType(),
      SageBuilder::buildExprListExp(SageBuilder::buildOpaqueVarRefExp(handle, SageInterface::getGlobalScope(node))),
      SageInterface::getScope(node));
