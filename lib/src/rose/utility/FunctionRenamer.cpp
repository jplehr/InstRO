...
class FunctionCallMatcher : public AstSimpleProcessing {
 public:
  FunctionCallMatcher(SgFunctionDeclaration *targetDecl) : targetDecl(targetDecl) {}
...
FunctionRenamer::FunctionRenamer(SgFunctionDeclaration *targetFunction) : targetFunction(targetFunction) {}

const FunctionRenamer::FunCallContainer &FunctionRenamer::findFunctionCalls(
    const std::vector<SgNode *> &startingPoints) {
...
const FunctionRenamer::FunCallContainer &FunctionRenamer::getFoundFunctionCalls() const { return functionCalls; }

void FunctionRenamer::renameFunctionDefinition(const std::string &newName) {
  SgFunctionDeclaration *definingDecl = targetFunction;
  // Function symbols are associated with the first non defining declaration, defining declarations do not have a symbol
  // themselves.
  ...
  // preceding statements calling the function need a fitting forward declaration when the defining declaration is
  // renamed:
...
void FunctionRenamer::redirectFunctionCall(SgFunctionCallExp *functionCall, SgFunctionDeclaration *newTarget) {
  ...
  SgFunctionSymbol *functionSymbol =
      lookupFunctionSymbolInParentScopes(newTarget->get_name(), newTarget->get_type(), stmt->get_scope());
  ...
  if (functionSymbol) {
    ...
    // sadly this does not work as intended in ROSE as generated declarations do not seem to have proper placement
    // information
