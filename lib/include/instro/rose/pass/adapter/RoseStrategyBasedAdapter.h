...
class RoseStrategyBasedAdapter : public RosePassImplementation {
 private:
  // priority-list
  std::vector<std::unique_ptr<InstRO::Rose::Adapter::StrategyBasedAdapterSupport::GenericInstrumentationStrategy>>
      strategyUniqPtrs;
  std::multimap<int, InstRO::Rose::Adapter::StrategyBasedAdapterSupport::GenericInstrumentationStrategy *,
                std::greater<int>>
      strategies;
  typedef std::multimap<int, InstRO::Rose::Adapter::StrategyBasedAdapterSupport::GenericInstrumentationStrategy *,
                        std::greater<int>>::iterator strat_iterator;

 protected:
  //				InstRO::Pass *input;
  template <class... stratList>
  void ctorStrategyInserter(
      std::unique_ptr<InstRO ::Rose::Adapter::StrategyBasedAdapterSupport::GenericInstrumentationStrategy> &&strat1,
      stratList &&... strategyArgs) {
 ...
 public:
  ...
  RoseStrategyBasedAdapter(
    ...
    int prio = 1;
    //					std::vector<std::uniq_ptr<InstRO::Rose::Adapter::StrategyBasedAdapterSupport::GenericInstrumentationStrategy>>
    // strategyVec = { strategyArgs... };
